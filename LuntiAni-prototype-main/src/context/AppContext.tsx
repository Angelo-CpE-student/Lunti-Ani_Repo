...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const applyPrediction = useCallback((next: Prediction) => {
    setPrediction(next);
  ...
  const simulateHighRiskSms = useCallback(() => {
    ...
    applyPrediction(next);
    setLastSms({
      body: `LuntiAni Alert\n\nHigh disease risk detected for your ${next.crop.toLowerCase()} crop.\n\nRisk: ${next.riskPercentage}%\nDisease: ${next.disease}\n\nPrototype preview only. No real SMS was sent.`,
      recipient: user?.contactNumber ?? "09XX XXX XXXX",
      timestamp: next.date,
    });
