...
function Toggle({ on, onChange, label }: { on: boolean; onChange: (v: boolean) => void; label: string }) {
  return (
    <button
      ...
      aria-label={label}
      onClick={() => onChange(!on)}
      className={`inline-flex h-7 w-12 shrink-0 items-center overflow-hidden rounded-full p-0.5 transition-colors ${on ? "bg-field" : "bg-line"}`}
    >
      <span
        className={`h-6 w-6 shrink-0 rounded-full bg-white shadow transition-transform ${
    ...
    <AppShell>
      <div className="mx-auto max-w-2xl px-5 md:px-8">
        ...
        <section className="mt-5">
          ...
          <p className="mt-2 text-xs text-ink-soft">Alerts are disabled in this prototype.</p>
        </section>
