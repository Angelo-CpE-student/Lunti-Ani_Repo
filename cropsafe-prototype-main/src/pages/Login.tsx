...
import { LeafIcon, UserIcon, LockIcon, AlertTriangleIcon } from "../components/Icons";
...
export default function Login() {
  ...
  function handleSubmit(e: FormEvent) {
    ...
    if (!ok) {
      setError("Incorrect username or password. Try demo / cropsafe.");
      setPassword("");
    }
  }
        ...
        <div className="absolute inset-0 flex items-end p-6 md:items-center md:p-12">
          <div className="text-white">
            <div className="flex items-center gap-2">
              <LeafIcon size={28} />
              <span className="font-display text-2xl font-bold">CropSafe</span>
            </div>
            <p className="mt-2 max-w-xs text-white/85">
              Crop disease risk monitoring for farmers.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-1 items-center justify-center px-6 py-10">
        <form onSubmit={handleSubmit} className="w-full max-w-sm" noValidate>
          ...
          <p className="mt-6 text-center text-sm text-ink-soft">
            ...
            <strong className="text-ink">cropsafe</strong>
          </p>
        </form>
      </div>
    </div>
